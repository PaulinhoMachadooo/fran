...
export default function Auth() {
  ...
  const getAuthErrorMessage = (error: any, fallback: string) => {
    ...
    if (lowered.includes("failed to fetch") || lowered.includes("network")) {
      return "Falha de conexão com o servidor de autenticação. Verifique a internet, a API e a variável VITE_API_URL.";
    }
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Erro no cadastro",
        description: getAuthErrorMessage(error, "Ocorreu um erro inesperado."),
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
