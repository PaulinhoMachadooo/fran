...
export default function PainelCliente() {
  ...
  const checkAuth = async () => {
    ...
    const sessionRes = await fetch(`${API_URL}/auth/me`, {
  ...
  const handleCreateAgendamento = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Enviar mensagem ao profissional via WhatsApp (wa.me)
      try {
        const funcionarioSelecionado = funcionarios.find(
          (f) => f.id === novoAgendamento.funcionario_id,
        );
        const servicoSelecionado = servicos.find(
          (s) => s.id === novoAgendamento.servico_id,
        );
        ...
        if (telefoneProfissional) {
          ...
          const tipoPetTexto =
            novoAgendamento.tipo_pet === "gato" ? "Gato" : "Cachorro";
          const mensagem =
            `Olá ${funcionarioSelecionado?.nome || ""}! Novo agendamento confirmado:\n\n` +
            `👤 Cliente: ${cliente.nome}\n` +
            `🐾 Pet: ${tipoPetTexto}\n` +
            `✂️ Serviço: ${servicoSelecionado?.nome || "Não informado"}\n` +
            `📅 Data: ${dataFormatada}\n` +
            `⏰ Horário: ${horaFormatada}`;
          window.open(
            `https://wa.me/${telefoneProfissional}?text=${encodeURIComponent(mensagem)}`,
            "_blank",
          );
        }
      } catch (waError) {
        console.error("Erro ao abrir WhatsApp:", waError);
      }
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Button
                      type="button"
                      variant={
                        novoAgendamento.tipo_pet === "cachorro"
                          ? "default"
                          : "outline"
                      }
                      onClick={() => {
                    ...
                    >
                      <Dog className="mr-2 h-4 w-4" /> Cachorro
                    </Button>
                    <Button
                      type="button"
                      variant={
                    ...
                    >
                      <Cat className="mr-2 h-4 w-4" /> Gato
                    </Button>
                  </div>
                  {novoAgendamento.tipo_pet === "gato" && (
                    <p className="text-xs text-muted-foreground">
                      Atendimento de gatos somente às terças e quartas, no
                      período da manhã.
                    </p>
                  )}
                </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            <h3
                              className={`font-semibold ${isCancelado ? "line-through text-muted-foreground" : ""}`}
                            >
                              {servicoDados.nome}
                            </h3>
                        ...
                        <div className="grid grid-cols-2 gap-2 text-sm">
                          <div>
                            ...
                            <p
                              className={`font-medium ${isCancelado ? "line-through text-muted-foreground" : ""}`}
                            >
